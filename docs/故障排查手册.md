...
**症状**:
```
[VALIDATION] verifying 验证失败 → 补偿回退到 executing
```

**原因**: 生成的代码未通过验证（py_compile / schema 检查 / 文件存在性）。

**解决方案**:
```bash
# 查看重试反馈
cat artifacts/retry_feedback.json

# 手动运行验证
python scripts/validator.py <task_dir>
```
...
| CFG-005 | 变量注入失败 | 检查 ${var} 占位符 |

### EXE - 执行错误 (可重试)

| 错误码 | 说明 | 重试策略 |
|--------|------|----------|
| EXE-101 | 网络超时 | 补偿回退后重试 |
| EXE-102 | API 限流 | 等待后重试 |
| EXE-103 | 认证失败 | **不可重试** |
| EXE-104 | 资源不足 | 等待后重试 |
| EXE-105 | 权限错误 | **不可重试** |
| EXE-106 | 临时故障 | 立即重试 |
...
graph TD
    ...
    ErrorCode -->|Other| UnknownIssue[其他问题]
